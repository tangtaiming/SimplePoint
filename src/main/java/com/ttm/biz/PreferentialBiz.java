...
public interface PreferentialBiz {
	...
	/**
	 * 修改保存 优惠数据 1.实现思路 1.1 获取优惠Map key是对应优惠id value是 对应优惠名称 1.2判断获取对应key 的value
	 * 是否有值，如果没有值 直接不进行任何操作，如果有数据接下来做1.3之后操作 1.3使用优惠id查询优惠数据，分两种情况 (1)查询有数据
	 * (2)查询没有数据 1.4(1)判断对应的优惠id是否有值，分两种情况(一)有值 :更新优惠数据 (二)没有值 保存数据
	 * 
	 * @param preferential
	 * @return
	 */
	 ...
	 * @return
	 */
	public abstract boolean saveOrUpdatePreferentialSimple(String name, String url);
